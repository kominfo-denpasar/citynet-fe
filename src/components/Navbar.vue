<template>
	...
	<section class="bg-gradient-to-r from-sky-200 to-blue-500 text-white py-2">
		...
		<nav aria-label="Social media" class="flex items-center gap-x-2"></nav>

		<div class="flex items-center gap-3">
			<div class="flex space-x-2">
			<button @click="setLang('en')" :class="activeLang === 'en' ? activeClass : inactiveClass">EN</button>
			<button @click="setLang('id')" :class="activeLang === 'id' ? activeClass : inactiveClass">ID</button>
			</div>

			<button
			@click="toggleDark"
			class="px-2 py-1 rounded-lg bg-gray-200 dark:bg-gray-700 text-xs md:text-sm text-gray-800 dark:text-gray-100"
			>
			{{ isDark ? 'Light' : 'Dark' }}
			</button>
		</div>
	...
	<nav aria-label="Main navigation">
		...
		<ul class="hidden md:flex gap-x-6 lg:gap-x-8 text-base md:text-lg text-gray-800 dark:text-gray-100 font-bold items-center">
			...
			>
				...
				<!-- Jika link eksternal -->
				<a
					v-if="sub.link && sub.link.startsWith('http')"
					:href="sub.link"
					target="_blank"
